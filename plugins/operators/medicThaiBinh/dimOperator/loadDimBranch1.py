...
class loadBranch1(BaseOperator):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        
    def execute(self, context):
        self.log.info("Start branch 1")
        ...
        #load table
        listDim = ["bacsi", "benhvien", "dichvu", "khoa", "phong", "gioitinh", "hinhthuctt"]
        for task in listDim:
            start = time.time()
            ...
            self.log.info(f"Done load {task}, took {round(end - start, 2)} second(s).")