...
class loadBranch2(BaseOperator):
    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)

    def execute(self, context):
        self.log.info("Start branch 2")
        ...
        listDim = ["nghenghiep", "dtvv", "benhravien", "benhrakhoa", "benhvaokhoa"]
        for task in listDim[:-2]:
        ...
        for task in listDim[-2:]:
            self.log.info(f"Start load {task}")
            start = time.time()
            newTask = loadType2(spark=spark, db=db, **connections[task])
            newTask.load()
            end = time.time()
            self.log.info(f"Done load {task}, took {round(end-start, 2)} (s)")
